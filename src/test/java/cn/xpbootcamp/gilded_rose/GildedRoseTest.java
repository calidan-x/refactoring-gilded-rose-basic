...
class GildedRoseTest {
    ...
    void should_update_item_correctly(TestFixture testFixture) {
        ...
        new GildedRose(new Product[] { item }).updateQuality();
    ...
    private static Stream<TestFixture> provideAgedBries() {
        return Stream.of(TestFixture.createAgedBrie(2, 0, 1, 1), TestFixture.createAgedBrie(2, 49, 1, 50),
                TestFixture.createAgedBrie(2, 50, 1, 50), TestFixture.createAgedBrie(0, 20, -1, 22),
                TestFixture.createAgedBrie(-1, 20, -2, 22));
    }

    private static Stream<TestFixture> provideBackstagePass() {
        return Stream.of(TestFixture.createBackstagePass(15, 20, 14, 21),
                TestFixture.createBackstagePass(11, 20, 10, 21), TestFixture.createBackstagePass(10, 45, 9, 47),
                TestFixture.createBackstagePass(9, 45, 8, 47), TestFixture.createBackstagePass(10, 49, 9, 50),
                TestFixture.createBackstagePass(10, 50, 9, 50), TestFixture.createBackstagePass(6, 45, 5, 47),
                TestFixture.createBackstagePass(5, 45, 4, 48), TestFixture.createBackstagePass(5, 49, 4, 50),
                TestFixture.createBackstagePass(1, 20, 0, 23), TestFixture.createBackstagePass(0, 20, -1, 0));
    }

    private static Stream<TestFixture> provideSulfuras() {
        return Stream.of(TestFixture.createSulfuras(-1, 50, -1, 50), TestFixture.createSulfuras(-1, 1, -1, 1),
                TestFixture.createSulfuras(-2, 1, -2, 1), TestFixture.createSulfuras(1, 1, 1, 1));
    }

    private static Stream<TestFixture> provideRegularItems() {
        return Stream.of(TestFixture.createRegular("+5 Dexterity Vest", 10, 20, 9, 19),
